...
from ..exceptions import AuthenticationError, AuthorizationError, ValidationError, DatabaseError
...
class UserService:
    ...
    def create_user(self, user_data: schemas.UserCreate, created_by: Optional[models.User] = None) -> models.User:
        """Create a new user."""
        try:
            # Validate input data
            ...
            existing_user = self.db.query(models.User).filter(
                models.User.username == validated_username
            ).first()
    ...
    def update_user(self, user_id: int, update_data: schemas.UserUpdate, updated_by: models.User) -> models.User:
        ...
        try:
            ...
            # Check permissions
            if updated_by.id != user_id and updated_by.role not in [models.RoleEnum.ADMIN, models.RoleEnum.SUPER_ADMIN]:
    ...
    def list_users(self, requesting_user: models.User, offset: int = 0, limit: int = 100) -> List[models.User]:
    ...
    def check_user_permissions(self, user: models.User, required_roles: List[models.RoleEnum]) -> bool:
