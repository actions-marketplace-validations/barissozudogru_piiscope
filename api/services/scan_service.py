...
from ..exceptions import ValidationError, FileProcessingError, DatabaseError, ResourceNotFoundError
from ..validators import validate_file_extension, validate_file_size, validate_scan_parameters, sanitize_filename
from ..logging_config import log_audit_event, log_performance_metric
from ..performance import timed, BatchProcessor
...
class ScanService:
    ...
    def create_scan_job(
        ...
        try:
            ...
            if 'file_path' in locals() and os.path.exists(file_path):
                os.remove(file_path)
            raise DatabaseError(f"Failed to create scan job: {str(e)}")
    ...
    def get_scan_job(self, job_id: int, user: models.User) -> Optional[models.ScanJob]:
        ...
        try:
            ...
            # Check permissions
            if job.user_id != user.id and user.role not in [models.RoleEnum.ADMIN, models.RoleEnum.SUPER_ADMIN]:
                raise ValidationError("Access denied to this scan job")
            
            return job
            
        except SQLAlchemyError as e:
            raise DatabaseError(f"Failed to get scan job: {str(e)}")
    
    def update_scan_progress(self, job_id: int, progress: float, status: models.ScanStatus = None) -> None:
    ...
    def add_scan_findings(self, job_id: int, findings: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            self.db.rollback()
            raise DatabaseError(f"Failed to add scan findings: {str(e)}")
    ...
    def set_scan_error(self, job_id: int, error_message: str) -> None:
        ...
        try:
            ...
            self.db.rollback()
            raise DatabaseError(f"Failed to set scan error: {str(e)}")
    ...
    def list_user_scans(
        ...
        try:
            ...
            return query.order_by(models.ScanJob.created_at.desc()).offset(offset).limit(limit).all()
            
        except SQLAlchemyError as e:
            raise DatabaseError(f"Failed to list scan jobs: {str(e)}")
    ...
    def get_scan_findings(
        ...
        try:
            ...
            return query.order_by(models.Finding.confidence.desc()).offset(offset).limit(limit).all()
            
        except SQLAlchemyError as e:
            raise DatabaseError(f"Failed to get scan findings: {str(e)}")
    ...
    def delete_scan_job(self, job_id: int, user: models.User) -> bool:
        ...
        try:
            ...
            self.db.rollback()
            raise DatabaseError(f"Failed to delete scan job: {str(e)}")
    ...
    def cleanup_expired_scans(self, max_age_hours: int = 24) -> int:
        ...
        try:
            ...
            expired_jobs = self.db.query(models.ScanJob).filter(
                ...
                models.ScanJob.status.in_([models.ScanStatus.COMPLETED, models.ScanStatus.FAILED])
            ).all()
            ...
            self.db.rollback()
            raise DatabaseError(f"Failed to cleanup expired scans: {str(e)}")