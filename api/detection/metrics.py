...
def compute_k_anonymity(
    ...
    Returns None if df is empty or no quasi-identifiers are provided.
    """
...
def compute_reidentification_risk(
    ...
    journalist_risk = round(
        float((1.0 / group_sizes).mul(group_sizes).sum() / n_total), 4
    )
    ...
    *,
    ...
    for col in sensitive_columns:
        ...
        if noise_mechanism == "laplace":
            ...
            delta = 1e-5
            epsilon = round(
                math.sqrt(2 * math.log(1.25 / delta)) * sensitivity / noise_scale, 4
            )
        else:
            epsilon = None
        results["columns"][col] = {
...
def _dp_epsilon_label(epsilon: Optional[float]) -> str:
    if epsilon is None:
        return "n/a"
    if epsilon < 0.1:
...
def generate_privacy_impact_assessment(
    ...
    reidentification_risk: Optional[Dict[str, Any]],
) -> Dict[str, Any]:
    ...
    retention_recs = {
        cat: rec["retention_days"]
        for cat, rec in masking_recs.items()
    }
